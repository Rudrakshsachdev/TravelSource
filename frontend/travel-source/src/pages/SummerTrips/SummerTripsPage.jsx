...
import { fetchAllSummerTrips } from "../../services/api";
import styles from "./SummerTripsPage.module.css";
...
/* ═══════════════════════════════════════════════════════════════
   Summer Getaways Full Page Component
   ═══════════════════════════════════════════════════════════════ */

const SummerTripsPage = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await fetchAllSummerTrips();
        setTrips(data);
      } catch (err) {
        setError(err.message || "Failed to load Summer trips.");
      } finally {
        setLoading(false);
      }
    })();
    ...
    <div className={styles.pageContainer}>
      ...
      <div className={styles.inner}>
        ...
        <header className={styles.pageHeader}>
          ...
          <h1 className={styles.heading}>Summer Getaways</h1>
          <p className={styles.subheading}>
            Discover our complete portfolio of seasonal escapes and vibrant summer journeys. 
            From pristine beaches to cool mountain retreats, curated for your perfect summer break.
          </p>
        </header>
        ...
        {loading ? (
          ...
          <div className={styles.emptyState}>
            <h2>No summer trips available right now.</h2>
            <p>Please check back later for new exclusive seasonal destinations!</p>
          </div>
        ) : (
          <div className={styles.grid}>
            {trips.map((trip, idx) => (
...
export default SummerTripsPage;
