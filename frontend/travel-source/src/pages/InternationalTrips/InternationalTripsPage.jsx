...
import { fetchAllInternationalTrips } from "../../services/api";
import styles from "./InternationalTripsPage.module.css";
...
/* ═══════════════════════════════════════════════════════════════
   International Trips Full Page Component
   ═══════════════════════════════════════════════════════════════ */
...
const InternationalTripsPage = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await fetchAllInternationalTrips();
        setTrips(data);
      } catch (err) {
        setError(err.message || "Failed to load international trips.");
      } finally {
        setLoading(false);
      }
    })();
    ...
    <div className={styles.pageContainer}>
      ...
      <div className={styles.inner}>
        ...
        <header className={styles.pageHeader}>
          ...
          <h1 className={styles.heading}>Explore All International Destinations</h1>
          <p className={styles.subheading}>
            Discover our complete portfolio of premium international getaways. 
            Carefully curated for the ultimate global adventure.
          </p>
        </header>
        ...
        {loading ? (
          ...
          <div className={styles.emptyState}>
            <h2>No international trips available right now.</h2>
            <p>Please check back later for new exclusive destinations!</p>
          </div>
        ) : (
          <div className={styles.grid}>
            {trips.map((trip, idx) => (
