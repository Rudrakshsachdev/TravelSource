...
import { fetchAllCommunityTrips } from "../../services/api";
import styles from "./CommunityTripsPage.module.css";
...
/* ═══════════════════════════════════════════════════════════════
   Community Trips Full Page Component
   ═══════════════════════════════════════════════════════════════ */
...
const CommunityTripsPage = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await fetchAllCommunityTrips();
        setTrips(data);
      } catch (err) {
        setError(err.message || "Failed to load Community excursions.");
      } finally {
        setLoading(false);
      }
    })();
    ...
    <div className={styles.pageContainer}>
      ...
      <div className={styles.inner}>
        ...
        <header className={styles.pageHeader}>
          ...
          <h1 className={styles.heading}>Wander Together</h1>
          <p className={styles.subheading}>
            Connect, explore, and create unforgettable memories with like-minded travelers. 
            Our community trips are designed for making friends while discovering new horizons.
          </p>
        </header>
        ...
        {loading ? (
          ...
          <div className={styles.emptyState}>
            <h2>No community trips available right now.</h2>
            <p>Please check back later for group adventures!</p>
          </div>
        ) : (
          <div className={styles.grid}>
            {trips.map((trip, idx) => (
                ...
                <div className={styles.content}>
                  ...
                  <div className={styles.cardFoot}>
                    ...
                    <span className={styles.stars}>
                      ...
                      <span className={styles.reviewCount}>(Community)</span>
                    </span>
                  </div>
                </div>
              </article>
