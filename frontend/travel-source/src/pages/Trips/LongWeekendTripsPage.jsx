...
import { fetchAllLongWeekendTrips } from "../../services/api";
import styles from "../Packages/PackagesPage.module.css"; // Reuse PackagesPage styles
...
const LongWeekendTripsPage = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await fetchAllLongWeekendTrips();
        setTrips(data);
      } catch (err) {
        setError(err.message || "Failed to load long weekend packages.");
      } finally {
        setLoading(false);
      }
    })();
      ...
      <div className={styles.pageContainer}>
        ...
        <div className={styles.inner}>
          <header className={styles.pageHeader}>
            ...
            <h1 className={styles.heading}>Long Weekend Gateways</h1>
            <p className={styles.subheading}>
              Hand-picked shorter trips perfect for those 3-4 day breaks.
              Curated locations within easy reach for a refreshing getaway.
            </p>
          </header>
          ...
          {loading ? (
            ...
            <div className={styles.emptyState}>
              <h2>No long weekend packages available right now.</h2>
              <p>Please check back later for new collection!</p>
            </div>
          ) : (
            <div className={styles.grid}>
              {trips.map((trip, idx) => (
