...
import { fetchAllGoodFridayTrips } from "../../services/api";
import styles from "./GoodFridayTripsPage.module.css";
...
/* ═══════════════════════════════════════════════════════════════
   Good Friday Trips Full Page Component
   ═══════════════════════════════════════════════════════════════ */

const GoodFridayTripsPage = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await fetchAllGoodFridayTrips();
        setTrips(data);
      } catch (err) {
        setError(err.message || "Failed to load Good Friday trips.");
      } finally {
        setLoading(false);
      }
    })();
    ...
    <div className={styles.pageContainer}>
      ...
      <div className={styles.inner}>
        ...
        <header className={styles.pageHeader}>
          ...
          <h1 className={styles.heading}>Explore All Good Friday Deals</h1>
          <p className={styles.subheading}>
            Discover our complete portfolio of premium Good Friday getaways. 
            Carefully curated for the long weekend adventure.
          </p>
        </header>
        ...
        {loading ? (
          ...
          <div className={styles.emptyState}>
            <h2>No Good Friday trips available right now.</h2>
            <p>Please check back later for new exclusive long weekend deals!</p>
          </div>
        ) : (
          <div className={styles.grid}>
            {trips.map((trip, idx) => (
...
export default GoodFridayTripsPage;
