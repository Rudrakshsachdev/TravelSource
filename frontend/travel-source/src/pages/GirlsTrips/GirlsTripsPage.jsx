...
import { fetchAllGirlsTrips } from "../../services/api";
import styles from "./GirlsTripsPage.module.css";
...
/* ═══════════════════════════════════════════════════════════════
   India Trips Full Page Component
   ═══════════════════════════════════════════════════════════════ */

const GirlsTripsPage = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await fetchAllGirlsTrips();
        setTrips(data);
      } catch (err) {
        setError(err.message || "Failed to load girls trips.");
      } finally {
        setLoading(false);
      }
    })();
    ...
    <div className={styles.pageContainer}>
      ...
      <div className={styles.inner}>
        ...
        <header className={styles.pageHeader}>
          ...
          <h1 className={styles.heading}>All Girls Group Tours</h1>
          <p className={styles.subheading}>
            Empowering journeys, deeper bonds, and unforgettable memories. Explore the world with like-minded women in a safe and vibrant community.
          </p>
        </header>
        ...
        {loading ? (
          ...
          <div className={styles.emptyState}>
            <h2>No girls group trips available right now.</h2>
            <p>Please check back later for new exclusively for women destinations!</p>
          </div>
        ) : (
          <div className={styles.grid}>
            {trips.map((trip, idx) => (
              ...
              >
                ...
                {trip.girls_featured_priority > 0 && (
                  <span className={styles.badge}>Girls Exclusive</span>
                )}
