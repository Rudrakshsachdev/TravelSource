...
import { fetchAllNorthIndiaTrips } from "../../services/api";
import styles from "./NorthIndiaTripsPage.module.css";
...
/* ═══════════════════════════════════════════════════════════════
   North India Trips Full Page Component
   ═══════════════════════════════════════════════════════════════ */

const NorthIndiaTripsPage = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await fetchAllNorthIndiaTrips();
        setTrips(data);
      } catch (err) {
        setError(err.message || "Failed to load North India trips.");
      } finally {
        setLoading(false);
      }
    })();
      ...
      <div className={styles.inner}>
        ...
        <header className={styles.pageHeader}>
          <span className={styles.eyebrow}>
            ...
            North India Collection
          </span>
          <h1 className={styles.heading}>Explore All North Indian Destinations</h1>
          <p className={styles.subheading}>
            Discover our complete portfolio of premium domestic getaways. 
            From the high Himalayas to the expansive plains, curated for the ultimate adventure.
          </p>
        </header>
        ...
        {loading ? (
          ...
          <div className={styles.emptyState}>
            <h2>No domestic trips available right now.</h2>
            <p>Please check back later for new exclusive destinations within North India!</p>
          </div>
        ) : (
          <div className={styles.grid}>
            {trips.map((trip, idx) => (
...
export default NorthIndiaTripsPage;
