...
import { fetchAllUttarakhandTrips } from "../../services/api";
import styles from "./UttarakhandTripsPage.module.css";
...
/* ═══════════════════════════════════════════════════════════════
   Uttarakhand Trips Full Page Component
   ═══════════════════════════════════════════════════════════════ */

const UttarakhandTripsPage = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const data = await fetchAllUttarakhandTrips();
        setTrips(data);
      } catch (err) {
        setError(err.message || "Failed to load Uttarakhand trips.");
      } finally {
        setLoading(false);
      }
    })();
    ...
    <div className={styles.pageContainer}>
      ...
      <div className={styles.inner}>
        ...
        <header className={styles.pageHeader}>
          ...
          <h1 className={styles.heading}>Journey to the Land of Gods</h1>
          <p className={styles.subheading}>
            Find yourself amidst the sacred rivers, ancient temples, and breathtaking alpine meadows of Uttarakhand.
          </p>
        </header>
        ...
        {loading ? (
          ...
          <div className={styles.emptyState}>
            ...
            <p>Please check back later for new exclusive destinations in Uttarakhand!</p>
          </div>
        ) : (
          <div className={styles.grid}>
            {trips.map((trip, idx) => (
...
export default UttarakhandTripsPage;
